...
class SpikerEnv(gym.Env):
  ...
  def __init__(self, chSel = 4, stimLimits = [0,2.1,0.1,10,1,1,10]):
    ...
    self.low = np.array([self.numPulsesLow,self.stimLow,self.PWLow,self.tBetPulsesLim])
    self.high = np.array([self.numPulsesHigh,self.stimHigh,self.PWHigh,200])
    self.action_space = gym.spaces.Box(self.low,self.high,dtype=np.float16)
    self.obsLow = np.zeros((1,self.numBins))
    ...
    #Define second order system filters. SecOrds - Bandpass 300-5000
  ...
  def step(self, stimSet):
    ...
    for ck in range(self.numStimTrials):
        ...
        sleep(1)                       #Give some time for the buffer to clear for writing.
        Waveform = Waveform[0:self.datalen-1]
        laser_buffer.write(Waveform)
        SpkBuf = self.circuitRZ2.get_buffer('SpkDat', 'r')
        data = SpkBuf.acquire('A', 'running', False)
        dataStore[str(ck)] = data[0][0][:]
    filtSignal = self.sigFilter(dataStore)
    spks = self.getSpikes(filtSignal)
    psth,psthKey = self.getPSTH(spks)
    try:
      psth = self.densityEstPSTH(psth)
    except:
      print('barsP error')
    reward = self.getScores(self.targetPSTH,psth)
    self.reward = reward
    self.obs = psth
    self.obsStore[str(self.counter)] = psth
